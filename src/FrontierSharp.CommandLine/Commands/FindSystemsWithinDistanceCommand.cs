...
public class FindSystemsWithinDistanceCommand(ILogger<FindSystemsWithinDistanceCommand> logger, IFrontierDevToolsClient devToolsClient, IAnsiConsole ansiConsole) : AsyncCommand<FindSystemsWithinDistanceCommand.Settings> {
    public override async Task<int> ExecuteAsync(CommandContext context, Settings settings) {
        ...
        if (systemsArray.Length == 0) {
            logger.LogError("No systems found within {maxDistance} of {systemName}", settings.MaxDistance, settings.SystemName);
            return 1;
        }

        var table = SpectreUtils.CreateAnsiTable($"Systems within {settings.MaxDistance} of {settings.SystemName}", "System", "Distance (LY)");
