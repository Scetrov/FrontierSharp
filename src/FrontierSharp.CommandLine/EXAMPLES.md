...
frontierctl system --name "Jita" | grep -A 10 "Smart Assemblies"
```

#### Chain Commands
```sh
# Get config and check a specific character
frontierctl config && frontierctl char --name "Player1"
```
...
```sh
# Run with Docker
docker run ghcr.io/scetrov/frontiersharp:latest tribe --name "MyTribe"

# Mount config file
docker run -v $(pwd)/config.json:/app/config.json ghcr.io/scetrov/frontiersharp:latest tribe --show-all
```
...
frontierctl tribe --id 42        # Precise query
```
