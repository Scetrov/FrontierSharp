using Microsoft.Extensions.Options;

namespace FrontierSharp.Data.Static;

public class FrontierResourceHive {
    private readonly FrontierResourceHiveOptions _options;
    private readonly string _resFiles;
    private readonly string _root;

    public FrontierResourceHive(string root, IOptions<FrontierResourceHiveOptions> options) {
        if (!Directory.Exists(root)) throw new DirectoryNotFoundException($"Directory not found: {root}");

        _resFiles = Path.Combine(root, "ResFiles");

        if (!Directory.Exists(_resFiles)) throw new DirectoryNotFoundException($"ResFiles not found in {root}");

        _root = root;
        _options = options.Value;
    }

    public ResIndex GetIndex(string? server = null) {
        server ??= _options.ServerName;

        var path = Path.Combine(_root, $"index_{server}.txt");

        if (!File.Exists(path)) throw new FileNotFoundException("Index not found", path);

        return new ResIndex(path);
    }

    public ResIndex GetResIndex(string? server = null) {
        var resIndex = GetIndex(server).FindByFilename(_options.ResFileIndexKey);
        return new ResIndex(ResolvePath(resIndex.RelativePath));
    }

    public FileStream GetNames() {
        var localeNames = GetResIndex().FindByFilename(_options.LocalisationMainKey);
        return File.OpenRead(ResolvePath(localeNames.RelativePath));
    }

    public FileStream GetTranslations() {
        var englishNames = GetResIndex().FindByFilename(GetLocalisationByCulture("en-us"));
        return File.OpenRead(ResolvePath(englishNames.RelativePath));
    }

    public string ResolvePath(string relativePath) {
        var path = Path.Combine(_resFiles, relativePath);

        if (!File.Exists(path)) throw new FileNotFoundException("Resource file not found", path);

        return path;
    }

    private string GetLocalisationByCulture(string culture) {
        return string.Format(_options.LocalisationEnglishPatten, culture);
    }
}